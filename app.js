...
const options = {
  ...
  connections.on('connection', async socket => {
	...
	const informConsumers = (roomName, socketId, id,viewer) => {
	  ...
	  producers.forEach(producerData => {
		if (  producerData.socketId !== socketId && producerData.roomName === roomName && producerData.admin) {
		  ...
		  producerSocket.emit('new-producer', { producerId: id ,socketId,viewer})
		}
	  })
	...
	socket.on("hosts:shareNewHosts",({roomName,waitingHosts})=>{
	  ...
	  let hostData=[]
	  producers.forEach(producerData => {
		if ( ! waitingHostSocket.includes(producerData.socketId) && producerData.roomName === roomName && producerData.producerId) {
	...
	socket.on("Call:DeclineCall",({producerId,roomName})=>{
		let socketId;
		producers.forEach(producerData => {
			if (  producerData.roomName === roomName && producerData.producer.id===producerId) {
		  ...
		  producers = removeItems(producers, socketId, 'producer')
				if(peers[socketId]){
					...
					rooms[roomName] = {
					  router: rooms[roomName].router,
					  peers: rooms[roomName].peers.filter(socketId_ => socketId_ !== socketId)
					}
				  }
	...
	socket.on('transport-produce', async ({ kind, rtpParameters, appData ,admin,viewer}, callback) => {
